...
const { GameServer } = require('./Class/Server.js');
...
gameServer.BroadcastMessageAll=BroadcastMessageAll;//server puede llamar broadcastall
...
wss.on('connection', function (socket) {
  ...
  //si entra alguien y el juego no ha empezado, pues que comience
  console.log(gameServer);
   ...
   socket.on('message', (data) => {
      ...
      //    console.log(tmpPlayer);
          ...
          playerList[playerID].color=tmpPlayer.color;
     //     console.log(playerList[playerID]);
          ...
          if(gameServer.hasGameStarted==false)
          {
            gameServer.BeginGame();
          }else
          {
            //sino el cliente debe informarse de donde esta el punto ahora y spawnearlo
            let networkPackage=new FNetworkPackage("ServerSpawnScorePoint", gameServer.networkClient, JSON.stringify( gameServer.scorePointPosition));
            socket.send(JSON.stringify(networkPackage));
          }
         return;
        }
        ...
        //  console.log(obj.client);
          ...
          //retorna verdadero si el player esta cerca del punto
          if(gameServer.CheckPlayerDistanceToPoint(playerPosition))
          {
            playerList[playerID].score++;
            //le avisamos a todos que el puntaje ha cambiado para este jugador

            let networkPackage=new FNetworkPackage("OnPlayerScoreChange",playerList[playerID].networkClient,playerList[playerID].score);
            BroadcastMessageAll(JSON.stringify(networkPackage));
